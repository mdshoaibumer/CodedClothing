      ...
      >
        ...
        <motion.div variants={itemVariants} className="pt-4 space-y-4 md:space-y-6">
          <div className="bg-white p-4 md:p-6 rounded-3xl md:rounded-4xl border border-obsidian-100 flex items-center justify-between shadow-soft">
            <span className="text-[9px] md:text-[10px] font-black text-obsidian-600 uppercase tracking-widest">Qty</span>
            <div className="flex items-center gap-4 md:gap-6">
              <motion.button
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.9 }}
                onClick={() => setQuantity(Math.max(1, quantity - 1))}
                className="w-10 h-10 md:w-12 md:h-12 rounded-full border-2 border-obsidian-100 flex items-center justify-center text-obsidian-600 hover:border-gold-500 hover:text-gold-600 transition-all font-black text-lg md:text-xl"
              >
                −
              </motion.button>
          ...
          {(design.front.logo || design.back.logo) && (
            <motion.button
              ...
              className="w-full py-4 rounded-2xl border-2 border-obsidian-100 text-obsidian-600 font-bold text-xs uppercase tracking-widest hover:border-gold-400 hover:text-gold-600 transition-all flex items-center justify-center gap-2"
            >
        ...
        {/* Craftsmanship Section — below order */}
        <motion.div variants={itemVariants} className="bg-gradient-to-br from-gold-50 to-amber-50 p-6 md:p-8 rounded-4xl border border-gold-100/50">
          ...
          >
            <h4 className="text-[9px] md:text-[10px] font-black text-gold-900 uppercase tracking-[0.2em] flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-gold-500 animate-pulse" />
              Editor Tips
            </h4>
            <span className={`text-gold-600 text-sm font-bold transition-transform duration-300 ${showTips ? 'rotate-180' : ''}`}>▾</span>
          </button>
          {showTips && (
            <ul className="space-y-2 md:space-y-3 mt-3 md:mt-4">
              {[
                ...
                <motion.li
                  ...
                  className="flex items-center gap-2 md:gap-3 text-[11px] md:text-xs font-bold text-gold-800"
                >
